...
OrderParameterRegistry::OrderParameterRegistry(
  const ParameterPack& input_pack, const DataSummary& data_summary
):
  data_summary_ptr_(&data_summary)
{
  ...
  for ( int p=0; p<num_ops; ++p ) {
    ...
    // TODO: allow relative paths, i.e.:  FileSystem::resolveRelativePath(tokens[0], <input-file-dir>);
    if ( tokens.size() > 1 ) {
...
int OrderParameterRegistry::nameToIndex(const std::string& name) const {
...
std::vector<int> OrderParameterRegistry::namesToIndices(const std::vector<std::string>& names) const {
